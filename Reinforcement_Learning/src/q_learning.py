...
class QLearning:
    ...
    def fit(self, env, steps=1000):
        ...
        for step in range(steps):
            ...
            if all([x == 0 for x in state_action_values[state, :]]) or prob < self._get_epsilon(step/steps):
                action_index = np.random.randint(N.shape[1])
            else:
                action_index = np.argmax(state_action_values[state, :])

            N[state][action_index] += 1
            alpha = 1/N[state][action_index]
